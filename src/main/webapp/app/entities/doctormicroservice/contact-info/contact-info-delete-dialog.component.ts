...
export class ContactInfoDeleteDialogComponent {
  ...
  constructor(private contactInfoService: ContactInfoService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class ContactInfoDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ contactInfo }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(ContactInfoDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
